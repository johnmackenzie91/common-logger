...
type ResponseResolver func(r *http.Response) (logrus.Fields, error)
...
// If successful the value for logrus.fields.request["body'] will be a json object,
// make it easier to query with something like jq. If json decode fails, this func falls back to ResolveResponse()
var ResolveJSONResponse = func(r *http.Response) (logrus.Fields, error) {
	f := logrus.Fields{}
	...
	bodyContent, err := readResponseBody(r)
...
// ResolveRequest writes request to logrus.Fields.request["request"]
var ResolveResponse = func(r *http.Response) (logrus.Fields, error) {
	f := logrus.Fields{}
...
// As the request body is a reader and can only be read once,
// we must refill for other processes further down the line who may wish to read it.
func readResponseBody(r *http.Response) (io.Reader, error) {
	// create a temp buffer
	b := bytes.NewBuffer(make([]byte, 0))
